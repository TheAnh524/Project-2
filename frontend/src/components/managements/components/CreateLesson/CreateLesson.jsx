...
import {
  createQuizLesson,
  createVideoLesson,
  FormCreateQuizLessonDto,
  FormCreateVideoLessonDto,
...
const CreateLesson = () => {
  ...
  const handleCreateVideoLecture = async (
    ...
    const createLessonData: FormCreateVideoLessonDto = {
      title: data.title,
      videoUrl: videoRes.data.url,
      topic: topicId,
    }

    return await createVideoLesson(createLessonData)
  }

  const handleCreateQuiz = async (data: z.infer<typeof CreateTopicSchema>) => {
    const createLessonData: FormCreateQuizLessonDto = {
      title: data.title,
      questions: data.questions!,
      topic: topicId,
    }

    return await createQuizLesson(createLessonData)
  }
  ...
  const onSubmit = async (data: z.infer<typeof CreateTopicSchema>) => {
    ...
    try {
      let createLessonRes
      if (data.type === LessonType.VideoLecture) {
        createLessonRes = await handleCreateVideoLecture(data)
      } else {
        createLessonRes = await handleCreateQuiz(data)
      }

      if (createLessonRes?.status === 201) {
        toast.success('Tạo bài học thành công.')
        navigate(-1)
        return
      }
      toast.error('Tạo bài học thất bại. Vui lòng thử lại sau.')
    } catch (error) {
      toast.error('Tạo bài học thất bại. Vui lòng thử lại sau.')
    } finally {
      setIsLoading(false)
  ...
  return (
    <div className="flex items-center flex-col">
      <h2 className="text-3xl font-bold mb-5">Tạo bài học mới</h2>
      <Form {...form}>
        <form
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder="Ví dụ: Ôn tập phép nhân" {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
